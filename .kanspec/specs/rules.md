...
- [rules.budget] `rules --budget <spec>` is the generator pointed at every tracked file under the spec's `code:` globs: rules, decisions and quirks in scope with their tokens, what prime would inject under the budget, and the surface in files; never an estimate of the work. A spec with no globs or no tracked files reports its own rules and says the surface is unknown. {p-67f0#c5}
- [rules.labels] `prime`'s live slice names every id by its label and opens with the NAMING line telling agents to do the same; the CLAUDE.md snippet carries the rule, and `show` accepts any t-/p-/D-/q- id so the label is one command away. {p-97d6}
