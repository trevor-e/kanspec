...
- [ladder.landed-derived] Landed is derived and never stored: the Done column is `state: done` AND the ladder says Landed (or a `--no-code` waiver or `repair` attestation in the file). An undetected close-out sits in Review wearing `closed out · awaiting merge`, does not satisfy a dependency, and a cache wipe moves it back to the chip, never to a wrong column. {p-97d6}
- [ladder.scan-done] A full scan covers `done` tickets until the ladder places them; a Merged fact on a terminal ticket carries over between scans, because a merge is permanent. {p-97d6}
