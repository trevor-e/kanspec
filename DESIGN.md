...
10. **Every state change appends to a per-ticket transition log with actor + verb**, and `kanspec doctor` replays that log against the transition table, so a state the trail does not reach fails CI. Be precise about what this buys, because the log is a plain text file anyone can edit and no claim beyond the following is being made:
    ...
    - **What is *detected*, not prevented.** A forger who edits the frontmatter *and* appends a matching log line produces a trail that replays perfectly. Nothing about the log itself can catch that — a `done` line proves a `done` line was written. What catches it is **corroboration against git**: every close the gate grants records the commit it was granted against, or the durable `--no-code` waiver, and neither can be minted without git or a signed reason. `doctor`'s `unproven_close` demands one of those (or a ladder run that saw the branch in main, or a `repair` attestation) of every `done` ticket that names a branch, a head or a PR, and fails CI without it.
