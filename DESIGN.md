...
**Cutting the tickets — annotated at review, decided at approve.** One ticket is one PR is one session: one capability, a handful of steps, sized S or M. `propose` pre-mints one `[tN] (spec: x)` per named capability, the indented sub-bullets under a `[tN]` mint as its `## Steps`, and `review`, `approve` and the page put a derived budget line under every item — the rules in scope for its capability in tokens (the `rules --budget` figure, from the generator prime spends) and the code under the spec's globs in files and lines, narrowed to the paths its `[cN]` bullets name. A cut that crosses a `[review]` threshold, spans two capabilities, is sized L, or leaves a `[cN]` with no ticket is flagged with the cut named; nothing blocks, and the human approving over a flag is the recorded waiver. kanspec never estimates the tokens the work will take: the line says what the ticket reads, and `surface unknown` when a spec names no code.
